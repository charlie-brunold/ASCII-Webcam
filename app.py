...
def image_to_layered_ascii(image, width=80, char_set='enhanced', sampling_factor=3, edge_detection=True, edge_threshold=50, force_unicode=False):
    ...
    for y in range(height):
        for x in range(width):
            ...
            # Use gaussian-weighted averaging for smoother results (base layer)
            block_height, block_width = block.shape
            weights = np.zeros((block_height, block_width))
            ...
            # Base layer - smooth gradients
            char_index = int(weighted_pixel * (len(ascii_chars) - 1) / 255)
            char_index = max(0, min(char_index, len(ascii_chars) - 1))
            base_str += ascii_chars[char_index]
            
            # Edge layer - directional characters where edges detected
            if edge_detection and edge_mask is not None:
                edge_block = edge_mask[start_y:end_y, start_x:end_x]
                edge_dir_block = edge_direction[start_y:end_y, start_x:end_x]
                ...
                if edge_ratio > 0.3:
                    ...
                    if np.any(edge_block):
                        ...
                        if len(edge_directions) > 0:
                            # Use the median direction for stability
                            dominant_direction = np.median(edge_directions)
                            edge_str += get_directional_char(dominant_direction)
                        else:
                            edge_str += ' '
                    else:
                        edge_str += ' '
            ...
            # Detail layer - high contrast areas and fine details
            if weighted_pixel > 200 and edge_detection and edge_mask is not None:
                edge_block = edge_mask[start_y:end_y, start_x:end_x]
                if np.mean(edge_block) > 0.2:
                    detail_str += '*'
                else:
                    detail_str += ' '
            elif weighted_pixel < 50 and edge_detection and edge_mask is not None:
                edge_block = edge_mask[start_y:end_y, start_x:end_x]
                if np.mean(edge_block) > 0.2:
                    detail_str += '.'
                else:
...
@app.route('/convert_layered', methods=['POST'])
def convert_image_layered():
    if 'image' not in request.files:
        return jsonify({'success': False, 'error': 'No image file provided'})
